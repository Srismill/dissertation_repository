...
#SAE car version of the experiment runfile, this one doesn't use the exploration phase. 

params = {}
params['update_interval'] = 20
params['compiler_iterations'] = 1
params['compiler_interval'] = 10
params['max_iterations'] = 1000 #Number of iterations the simulation runs for, changed for some exploration studies
params['meeting_length'] = 50
params['reward_scale'] = 1

#Exp 2 parameters, not used in this study.
params['subteam_size'] = 1
params['joint_iteration'] = 0
params['partial_accept'] = 0
params['use_subsets'] = 0
params['sync_merges'] = 0
params['subset_size'] = 1
params['nominal_PB'] = 0

params['VC_agents'] = 6 #not used for SAE car

exploration_phase_fraction = 0.01 #If you want to use the exploration phase, increase this to whatever fraction of the process runtime you want it to go for.
...
for i in range(len(max_designs)):
    ...
    for k in range(num_tries):
        ...
        test_framework.problem.weights[-1] *= 1 #Pitch moment multiplier; 1 for uncoupled, 100 for mixed, 1000 for coupled
...
r_2 = obj_record2
np.save('sae_exp00_uncoupled',obj_record2)
